...
def insert_regular_333_bld_solves():
    ...
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    csv_file = glob.glob(os.path.join('txt_files', "333ni*_solves_*.csv"))[0]
...
def insert_regular_555_bld_solves():
    ...
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    csv_file = glob.glob(os.path.join('txt_files', "555bld*_solves_*.csv"))[0]
...
def insert_edges_corners_only_555_bld_solves():
    db_file = "db_solves\\all_solves.db"
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    csv_file = glob.glob(os.path.join('txt_files', "5edge*_solves_*.csv"))[0]
...
def main():
    ...
    if args.scramble_type == "5edge":
        insert_edges_corners_only_555_bld_solves()
