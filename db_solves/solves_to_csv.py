...
def convert_corners_only_333_solves_to_csv():
    ...
    print(f"CSV file saved to {output_file}")
def convert_edges_only_333_solves_to_csv():
    
    input_file = glob.glob(os.path.join('txt_files', "edges*_solves_*.txt"))[0]
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join('txt_files', "edges_solves_{}.csv".format(current_time)) 
    print(input_file)
    columns = ["scramble_type","scramble","edge_buffer", "edges", "flip", "first_lp_edges_join"]
    data = []
    ...
    with open(input_file, "r", encoding="utf-8") as file:
        for line in file:
            ...
            edge_buffer = key_values.get("edge_buffer", "").strip()
            # Add the parsed row to the data list
            data.append([scramble_type,scramble,edge_buffer,edges, flip, first_edges])
...
def convert_regular_555_solves_to_csv():
    input_file = glob.glob(os.path.join('txt_files', "555bld*_solves_*.txt"))[0]
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join('txt_files', "555bld_solves_{}.csv".format(current_time)) 
    print(input_file)
...
def convert_edges_corners_only_555_solves_to_csv():
    input_file = glob.glob(os.path.join('txt_files', "5edge*_solves_*.txt"))[0]
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join('txt_files', "5edge_solves_{}.csv".format(current_time)) 
    print(input_file)
    columns = ["scramble_type","scramble","edge_buffer", "corner_buffer", "edges", "flip", "corners", "Twist Clockwise", "Twist Counterclockwise", "first_lp_edges_join", "first_lp_corners_join", "wing_buffer", "wings", "first_lp_wings_join", "xcenter_buffer","xcenters", "first_lp_xcenters_join", "tcenter_buffer", "tcenters", "first_lp_tcenters_join"]
    data = []
...
def main():
    ...
    if args.scramble_type == "edges":
        convert_edges_only_333_solves_to_csv()
    if args.scramble_type == "555bld":
        convert_regular_555_solves_to_csv()
    if args.scramble_type == "5edge":
        convert_edges_corners_only_555_solves_to_csv()
